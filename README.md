...
> 발표나 녹화를 직접 돌린다면 [docs/DEMO.md](docs/DEMO.md) 에 진행표가 있다.
> 시작 전 점검, 두 모드의 차이, 막혔을 때 할 일까지 적어 두었다.
...
그 키가 한 브라우저에만 있다는 것은 **브라우저 데이터를 지우면 그
컨트랙트의 발급 권한이 영구히 사라진다**는 뜻이다. 원장의 `issuerPk` 는
배포 때 고정됐고 회로가 그걸 보므로, 새 키를 만들어도 채권을 넣을 수 없다.
컨트랙트를 다시 배포하는 것 말고는 방법이 없다.

`/devtools` 의 **발급 기관 키** 패널에서 내보내고 가져온다.

- **내보내기** — 비밀키·유도한 공개키·네트워크·컨트랙트 주소를 JSON 으로
  내려받는다. 파일명에 컨트랙트 주소 앞자리가 들어가 어느 배포의 것인지
  열어 보지 않고 안다 (`once-issuer-key-52a72d93.json`).
- **가져오기** — 파일에 적힌 공개키를 믿지 않고 **비밀키에서 다시 유도해**
  원장의 `issuerPk` 와 대조한 결과를 그 자리에서 보여준다. 안 맞으면
  "이 키는 컨트랙트 0x52a7… 의 발급 기관이 아니다" 라고 적는다.

가져오기에는 안전장치가 하나 있다. 지금 이 기기의 키가 원장과 맞는데
들어온 키가 아니면 **넣지 않는다.** 백업이 없어 권한을 잃는 것을 막으려고
만든 기능이 바로 그 사고를 내면 안 된다. 그때는 먼저 내보내라고 말한다.

나가는 것은 **서명키 하나뿐이다.** 채권 원문·salt·소유자 비밀키가 든
비공개 상태 저장소는 여전히 내보낼 수 없고 그 경로를 만들지 않는다.

> 파일은 평문이다. 가진 사람은 그 컨트랙트에 채권을 발급할 수 있다.
> `.gitignore` 가 `once-issuer-key-*.json` 을 막아 두었지만, 파일 자체를
> 어디에 두는지는 사람이 관리해야 한다.
