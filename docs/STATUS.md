...
**G4(테스트넷 배포)를 제외한 전부가 끝났다. G4는 지갑 동기화에 약 8시간이
더 필요하고, 브라우저 + Lace 경로로 가면 그 8시간이 통째로 사라진다.**

## 오늘의 결론: dust 동기화가 병목이다

세 지갑 중 shielded(1,530,192)와 unshielded는 완료됐다. **dust만 남았고,
dust도 같은 1,530,262 범위를 동기화해야 한다.**

```
dust 90,541 / 1,530,262 (5.9%)   ← 8회차 시점
```
...
감독 루프 상한을 200회로 올려 밤새 돌 수 있게 했다. CPU는 1~2코어만 쓴다.

**다만 이 8시간은 브라우저 + Lace 경로에서는 존재하지 않는다.** Lace가
자기 상태를 스스로 동기화하고 배포 수수료도 낸다 (아래 S6-d).
