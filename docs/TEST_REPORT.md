...
`pnpm test`: **14개 파일, 50건 전부 통과** (2026-09-17 기준).
...
- **테스트넷 배포와 실제 증명 생성** (G4). 로컬 실행은 회로 로직을 직접
  ...
  재현 스크립트는 준비돼 있다 (`apps/deploy/src/scenario.ts`). 배포 후
  A5(동시 신청)·A6(지연 제출)을 실제 노드에서 돌리고 단계별 tx 해시와
  소요 시간을 `scenario-result.json`에 기록한다.

  현재 막혀 있는 지점은 **첫 지갑 동기화**다. Preprod는 인덱스가
  1,529,726개이고 초당 약 200개씩 처리해 **두 시간 남짓** 걸린다.
  자세한 측정치는 [DEPLOY.md](DEPLOY.md) §3에 있다.
